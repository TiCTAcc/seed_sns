...
if (!empty($_POST) && isset($_POST)) {
  $sql_select='SELECT * FROM `members` WHERE `email`=? AND `password`=?';
  $data_select=array($_POST['email'],sha1($_POST['password']));
  $stmt_select=$dbh->prepare($sql_select);
  $stmt_select->execute($data_select);

  $member=$stmt_select->fetch(PDO::FETCH_ASSOC);
