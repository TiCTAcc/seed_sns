<?php
 ...
 // var_dump($_POST);
  ...
  if (!empty($_POST)) {
    ...
    if (!isset($error)) {
  //(1) COUNT( ) SQL文の関数。ヒットした数を取得する。
  // emailの重複を避けたい
  // (2) AS句

$sql_select='SELECT COUNT(*) AS `mail_count` FROM `members` WHERE `email`=?';
$data_select=array($_POST['email']);
$stmt_select=$dbh->prepare($sql_select);
$stmt_select->execute($data_select);
// (5)fetch忘れすぎ

$mail_count =$stmt_select->fetch(PDO::FETCH_ASSOC);
        ...
        <form method="post" action="" class="form-horizontal" role="form" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-8">
              ...
              <p class="error">* メールアドレスが重複しています。メールアドレスを変更してください。</p>
              <?php } ?>
            </div>
          </div>
          <!-- パスワード -->
